"""
PGD-BERT Attack for Hugging Face models
------------------------------------------
Implements a gradient-driven word substitution attack for Hugging Face sequence classification models.
"""
...
def main():
    parser = argparse.ArgumentParser(description="PGD-BERT Attack for Hugging Face sequence classification models")
    parser.add_argument("--model_name", type=str, required=True, help="Hugging Face model name (e.g., 'bert-base-uncased' or 'microsoft/DialoGPT-medium')")
    ...
    # For MLM, we'll use the same base model if it supports masked language modeling
    # Otherwise, we'll use a compatible MLM model
    try:
        mlm = AutoModelForMaskedLM.from_pretrained(args.model_name).to(device).eval()
        mlm_tokenizer = AutoTokenizer.from_pretrained(args.model_name)
    except:
        # Fallback to bert-base-uncased for MLM if the main model doesn't support it
        print(f"Warning: {args.model_name} doesn't support masked language modeling. Using bert-base-uncased for MLM.")
